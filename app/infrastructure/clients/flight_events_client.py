...
class FlightEventsClient:
    ...
    async def get_events(self) -> list[FlightEvent]:
        ...
        try:
            ...
            raise UpstreamUnavailableError("Flight-events API timed out")
        except httpx.ConnectError as exc:
            logger.error("Cannot connect to upstream", extra={"error": str(exc)})
            raise UpstreamUnavailableError("Cannot connect to flight-events API")
    ...
    def _parse(self, data:list[dict]) -> list[FlightEvent]:
        ...
        for item in data:
            try:
                ...
                raise UpstreamInvalidPayloadError(f"Unexpected upstream schema: {exc}")
            except ValueError as exc:
                logger.error("Upstream returned incoherent data", extra={"error": str(exc)})
                raise UpstreamInvalidPayloadError(str(exc))
